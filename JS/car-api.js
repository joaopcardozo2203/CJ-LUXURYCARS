...
async function buscarDadosCarroAPI(make, model) {
    try {
        const endpoint = `${NHTSA_BASE_URL}/GetModelsForMake/${encodeURIComponent(make)}?format=json`;
        ...
        console.warn("Erro ao consultar a API de veículos (usando fallback local):", error);
